...
final class SMTPResponseDecoder: ChannelInboundHandler {
    ...
    func channelRead(context: ChannelHandlerContext, data: NIOAny) {
        ...
        case ("2", " "), ("3", " "): context.fireChannelRead(wrapInboundOut(.success((code, remainder))))
        case (_, "-"): break // intermediate message, ignore
        default: context.fireChannelRead(wrapInboundOut(.failure(ServerError(serverMessage: firstFourBytes + remainder))))
        }
    }
}
